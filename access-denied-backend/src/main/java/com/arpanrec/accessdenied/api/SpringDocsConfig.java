...
public class SpringDocsConfig {
    ...
    public OpenAPI customize() {
        ...
        var securityItem = new SecurityScheme()
                ...
                .name(AccessDeniedConstants.API_KEY_HEADER);
        var securityComponents =
                new Components().addSecuritySchemes(AccessDeniedConstants.OPENAPI_SECURITY_SCHEME_NAME, securityItem);
    ...
    public OpenApiCustomizer customApiWithNamespaceHeader() {
        return openApi -> openApi.getPaths().forEach((path, pathItem) -> {
            if (!path.toLowerCase().startsWith(AccessDeniedConstants.API_ENDPOINT.toLowerCase())) {
                return;
            }
            if (path.toLowerCase()
                    .startsWith(AccessDeniedConstants.API_ENDPOINT + "/" + AccessDeniedConstants.INIT_ENDPOINT)) {
                return;
            }
            log.debug("Adding namespace header to path: {}", path);
            pathItem.addParametersItem(new Parameter()
                    .name(AccessDeniedConstants.NAMESPACE_HEADER)
                    .in(ParameterIn.HEADER.name())
                    .required(false)
                    .schema(new StringSchema()._default(AccessDeniedConstants.NAMESPACE_DEFAULT))
