...
@Log4j2
@RestController
@RequestMapping(path = AccessDeniedConstants.API_ENDPOINT + "/token")
...
public class TokensApi {
    ...
    public ApiKey refreshToken(@RequestBody ApiKeyRefreshRequest apiKeyRefreshRequest) {
        ...
        if (newExpiration > System.currentTimeMillis() + AccessDeniedConstants.MAX_API_KEY_MILI_SECONDS) {
            throw new RuntimeException("API key validity cannot exceed "
                    + AccessDeniedConstants.MAX_API_KEY_MILI_SECONDS + " " + "milliseconds");
        }
